  ...
  if((vehicle player) != player) exitWith { hint "You can't change your clothes while in a vehicle"; };
  {
    ...
    if (_type == (_this select 1)) exitWith {
      ...
      if (!isnil ("_newUnit")) then {
        ...
        if(count _x == 4) then { _plrInit = _plrInit + format["this setObjectTexture %1;this setObjectTexture %2", (_x select 2), (_x select 3)]; 
		respawnSkin = (_x select 2); respawnSkinb = (_x select 3); 
		}; 
		if(count _x == 5) then { _plrInit = _plrInit + format["this setObjectTexture %1;this setObjectTexture %2;this setObjectTexture %3",(_x select 2),(_x select 3),(_X select 4)]; 
...
if(_action == "respawn" && !isNil("respawnSkin") && isNil("respawnSkinb") && isNil("respawnSkinc")) exitWith {
  player setVehicleInit format["this setObjectTexture %1;",respawnSkin];
  processInitCommands;
};
if(_action == "respawn" && !isNil("respawnSkin") && !isNil("respawnSkinb") && isNil("respawnSkinc")) exitWith {
  player setVehicleInit format["this setObjectTexture %1;this setObjectTexture %2",respawnSkin,respawnSkinb];
  processInitCommands;
  };
if(_action == "respawn" && !isNil("respawnSkin") && !isNil("respawnSkinb") && !isNil("respawnSkinc")) exitWith {
  player setVehicleInit format["this setObjectTexture %1;this setObjectTexture %2;this setObjectTexture %3",respawnSkin,respawnSkinb,respawnSkinc];
  processInitCommands;
  };
