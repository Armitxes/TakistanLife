...
if (!(_playerobject call ISSE_UnitExists))                exitWith {player groupChat localize "STRS_inv_not_ingame";};

if (_playerobject == player)                         exitWith {player groupChat localize "STRS_inv_inventar_uebergabe_self";};
//call compile format['if (%1 == player) exitWith {player groupChat localize "STRS_inv_inventar_uebergabe_self";};', _spieler];
...
//if (!(_spieler in _closeplayers) and (player distance _playerobject > 20))              exitWith {player groupChat localize "STRS_inv_inventar_uebergabe_distance";};
...
//player groupChat format [localize "STRS_inv_inventar_uebergabe_success_self", name (call compile _spieler), (_menge call ISSE_str_IntToStr), _itemanzeige];
