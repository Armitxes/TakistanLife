...
if (_art == "deposit") then {
  ...
  if ((PLAYERDATA select 1) < _steuern) then {
    ...
    (format ["if (INV_ROLESTRING == ""%1"") then {[%2] call setMoney; player groupChat format[localize ""STRS_bank_account_getmoney"", %2, %3];};", _targetString, (_transfermoney call ISSE_str_IntToStr), player]) call toClients;
	};
};
